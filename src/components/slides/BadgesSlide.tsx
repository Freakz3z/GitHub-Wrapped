...
export default function BadgesSlide({ data }: Props) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-6xl mx-auto max-h-[calc(100vh-200px)] overflow-y-auto overflow-x-hidden pb-20">
        <motion.h2
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-3 md:mb-4 bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent"
        >
          Achievement Badges
        </motion.h2>
        <motion.p
          ...
          className="text-center text-base md:text-xl text-gray-400 mb-6 md:mb-8"
        >
        ...
        >
          <h3 className="text-lg md:text-2xl font-bold text-white mb-4 md:mb-6 flex items-center justify-center gap-2">
            <span className="text-yellow-400 text-2xl md:text-3xl">🏆</span> Unlocked Badges
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            {unlockedBadges.map((badge, index) => (
              <motion.div
                ...
                className="p-4 md:p-5 bg-gradient-to-br from-yellow-500/10 to-orange-500/10 backdrop-blur-sm rounded-xl md:rounded-2xl border-2 border-yellow-500/50 cursor-pointer"
              >
        ...
        {lockedBadges.length > 0 && (
          ...
          >
            <h3 className="text-lg md:text-2xl font-bold text-gray-400 mb-4 md:mb-6 flex items-center justify-center gap-2">
              <Lock className="w-5 h-5 md:w-6 md:h-6" /> Locked Badges
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              {lockedBadges.map((badge, index) => (
                ...
                >
                  <div className="text-3xl md:text-4xl mb-2 text-center opacity-30 grayscale">{badge.icon}</div>
